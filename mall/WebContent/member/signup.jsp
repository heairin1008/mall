...
<script>
	...
	$(document).ready(function(){
		$("#signupBtn").click(function(){
			if($("#memberEmail").val() == ""){
				alert("이메일을 확인해주세요");
				$("#memberEmail").focus(); // 커서를 이메일 입력 창으로 이동
				return;
			}else if($("#memberPw").val() == ""){
				alert("비밀번호를 확인해주세요");
				$("#memberPw").focus(); // 커서를 비밀번호 입력 창으로 이동
				return;
			}else if($("#memberName").val() == ""){
				alert("이름을 확인해주세요"); 
				$("#memberName").focus(); // 커서를 이름 입력 창으로 이동
				return;
			}
			$("#signupForm").submit();
		});
...
<div class="row" style="margin : 10px;"> <!-- 헤더 goodee shop / 검색바 -->
	...
	<form method="post" action="<%=request.getContextPath()%>/member/signupAction.jsp" id="signupForm">
		...
		<button class="btn btn-info" type="button" id="signupBtn">회원가입</button>
		</div>
	</form>
</div>
</body>
